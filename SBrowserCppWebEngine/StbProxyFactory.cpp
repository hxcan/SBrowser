...
StbProxyFactory::StbProxyFactory()
{
  ...
  loadBuiltinAdBlockUrlSet(); // 载入需要封锁的URL集合。 Builtin set.
...
QList<QNetworkProxy> StbProxyFactory::queryProxy(const QNetworkProxyQuery &query)
{
    ...
    else //该URL不要封锁，则按照正常流程来判断。尽力让浏览器能够连接到它。
    {
        ...
        if (usngSmtPrxy) //要使用智能代理。
        {
            ...
            {
                if (query.queryType()==QNetworkProxyQuery::TcpSocket) //建立TCP连接。
                {
                    result << smtPrxyObj; //加入智能代理。
                } //if (query.queryType()==QNetworkProxyQuery::TcpSocket) //建立TCP连接。
                else //不是建立TCP连接。
                {
                  if (prHstNm.isEmpty()) // 主机名为空。
                  {
                  } //if (prHstNm.isEmpty()) //主机名为空。
                  else // 主机名不为空。
                  {
...
/*!
 * \brief StbProxyFactory::loadBuiltinAdBlockUrlSet 载入需要封锁的URL集合。 Builtin set.
 */
void StbProxyFactory::loadBuiltinAdBlockUrlSet()
{
  QString ssnStrJs=":/builtin.data.sound/builtin.ad.block.urls.i.json"; // 要封锁的URL集合记录的JSON文件名。

  loadAdBlockUrlSetFromFile(ssnStrJs); // Load from file.
} // void StbProxyFactory::loadBuiltinAdBlockUrlSet()

/*!
 * \brief StbProxyFactory::loadAdBlockUrlSetFromFile Load from file.
 * \param ssnStrJs The file path.
 */
void StbProxyFactory::loadAdBlockUrlSetFromFile(const QString & ssnStrJs)
{
  QFile ssnStrJsFl(ssnStrJs); //JSON文件。
  ssnStrJsFl.open (QIODevice::ReadOnly); //打开文件。
  QByteArray ssnStrBtAr=ssnStrJsFl.readAll (); //全部读入。
  ...
  auto json=QString::fromUtf8 (ssnStrBtAr); //将JSON内容转换成字符串。
...
QStringList StbProxyFactory::getAdBlockUrls()
{
  ...
  result=adBlckUrlSt.values (); //转换成列表。
...
bool StbProxyFactory::shouldBlockUrl(QString peerUrl)
{
    ...
    if (_enableAdBlock) //启用广告禁用功能。
    {
        ...
        {
//            QRegExp rx(crtBlckUrlReg); //正则式。
          auto convertedExpression=QRegularExpression::wildcardToRegularExpression (crtBlckUrlReg);

            QRegularExpression rx(convertedExpression); //正则式。
//            rx.setPatternSyntax(QRegExp::Wildcard); //设置为通配符模式。

//            QRegExpValidator v(rx,0); //校验器。
            QRegularExpressionValidator v(rx, 0);
            int pos=0; //匹配的起始位置。

            if (v.validate(peerUrl,pos)==QValidator::Acceptable) //匹配这个正则式。
            {
                result=true; //应当封锁。
